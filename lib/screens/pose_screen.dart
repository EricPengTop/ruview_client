...
class PoseScreen extends ConsumerWidget {
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              return ChoiceChip(
                label: Text(
                  '${s.getString("pose_target")}${p.trackId} (${(p.confidence * 100).toStringAsFixed(0)}%)',
                  style: const TextStyle(fontSize: 12),
                ),
                selected: true,
                onSelected: (_) => _showDetail(context, p, s),
  ...
  void _showDetail(BuildContext context, PoseDetection person, AppStrings s) {
    showModalBottomSheet(
      context: context,
      builder: (_) => _PoseDetailSheet(person: person, s: s),
    );
  }
}
...
class _PoseDetailSheet extends StatelessWidget {
  ...
  List<String> get _keypointNames => [
    s.getString('pose_kp_nose'),
    s.getString('pose_kp_left_eye'),
    s.getString('pose_kp_right_eye'),
    s.getString('pose_kp_left_ear'),
    s.getString('pose_kp_right_ear'),
    s.getString('pose_kp_left_shoulder'),
    s.getString('pose_kp_right_shoulder'),
    s.getString('pose_kp_left_elbow'),
    s.getString('pose_kp_right_elbow'),
    s.getString('pose_kp_left_wrist'),
    s.getString('pose_kp_right_wrist'),
    s.getString('pose_kp_left_hip'),
    s.getString('pose_kp_right_hip'),
    s.getString('pose_kp_left_knee'),
    s.getString('pose_kp_right_knee'),
    s.getString('pose_kp_left_ankle'),
    s.getString('pose_kp_right_ankle'),
  ];
          ...
          Flexible(
            child: ListView.builder(
              ...
              itemBuilder: (context, i) {
                ...
                final name = i < _keypointNames.length
                    ? _keypointNames[i]
                    : '#$i';
                return Padding(
...
class _SkeletonPainter extends CustomPainter {
  ...
  void _drawSkeleton(Canvas canvas, Size size, PoseDetection person) {
    ...
    for (final bone in _bones) {
      canvas.drawLine(joints[bone[0]], joints[bone[1]], _bonePaint);
    }
