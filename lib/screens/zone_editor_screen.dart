    ...
    return Scaffold(
      ...
      body: Column(children: [
        Container(
          ...
          child: Row(children: [
            Icon(Icons.touch_app, size: 16, color: Colors.grey.shade400), const SizedBox(width: 8),
            Text(s.format('z_editor_hint', args: {'count': '${_points.length}'}), style: TextStyle(fontSize: 13, color: Colors.grey.shade400)),
            const Spacer(),
            if (_points.length >= 3) Text(s.getString('z_editor_closable'), style: TextStyle(fontSize: 12, color: Colors.green.shade300)),
          ]),
        ),
        Expanded(
          child: GestureDetector(
            onTapDown: (d) => _addPoint(d.localPosition),
            child: CustomPaint(size: Size.infinite, painter: _ZonePainter(points: _points)),
          ),
        ),
      ]),
    );
...
class _ZonePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      final tp = TextPainter(text: TextSpan(text: '${i + 1}', style: const TextStyle(color: Colors.white, fontSize: 10)), textDirection: TextDirection.ltr);
      tp.layout(); tp.paint(canvas, points[i] + const Offset(8, -16));
    }
  }
